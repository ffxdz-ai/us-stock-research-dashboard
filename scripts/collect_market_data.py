...
def load_local_futu_snapshot_quotes(symbols: list[str], base_quotes: dict[str, dict[str, Any]]) -> dict[str, dict[str, Any]]:
    """Use a recent local Futu snapshot when live OpenD is unavailable.

    The snapshot is generated by scripts/sync_futu_local_snapshot.py and contains
    only public quote fields. It is a local enhancement layer; cloud jobs simply
    skip it when the file is absent.
    """
    try:
        max_age_hours = float(os.getenv("FUTU_LOCAL_SNAPSHOT_MAX_AGE_HOURS", "12"))
    except ValueError:
        max_age_hours = 12.0
    payload = load_fresh_json(LOCAL_FUTU_SNAPSHOT_PATH, max_age_hours)
